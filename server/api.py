...
async def lipsync_endpoint(
    ...
    Returns a fragmented MP4 video stream as a StreamingResponse (suitable for Media Source Extensions).
    """
    try:
        ...
        # Create or get the avatar (preparation may occur if needed).
        avatar = await run_in_threadpool(get_or_create_avatar, avatar_id, video_path, bbox_shift, batch_size)
        
        # Get the fragmented MP4 stream from the inference_mse method.
        stream_generator = avatar.inference_mse(str(audio_temp_path), fps=DEFAULT_FPS)
...
async def create_avatar_endpoint(
    ...
    The video is saved and then used to create the avatar.
    """
    try:
